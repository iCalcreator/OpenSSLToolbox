...
abstract class OpenSSLBaseFactory2 extends OpenSSLBaseFactory
{
    ...
    protected static function assertConfig( array $configArgs ) {
        ...
        if( ! is_scalar( $configArgs[self::PRIVATEKEYBITS] )) {
            throw new InvalidArgumentException( sprintf( $FMT, var_export( $configArgs[self::PRIVATEKEYBITS], true )));
        }
        ...
        if( 384 > $keyBits ) {
            throw new InvalidArgumentException( sprintf( $FMT, $configArgs[self::PRIVATEKEYBITS] ));
        }
    }
     ...
     * @return bool
     */
    public function isConfigSet( $key = null ) {
