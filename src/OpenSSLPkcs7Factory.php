...
class OpenSSLPkcs7Factory extends OpenSSLBaseFactory
{
     ...
     */
    public static function assertFlags(
        $flags,
        $argIx = 1,
        $valueIfNull = null
    ) : int
     ...
     * @return string
     */
    private static function getUniqueFilenameBase() : string
    {
        static $SP0 = '';
    ...
    ) : bool
    {
        ...
        if( is_resource( $recipCert )) {
            $recipCert = OpenSSLX509Factory::factory( $recipCert )
     ...
     * @param null|resource|string|array $recipKey
     *                             1. A key resource returned from openssl_get_publickey() or openssl_get_privatekey()
     *                             2. A string having the format (file://)path/to/file.pem
     *                                The named file must contain a PEM encoded certificate/private key (it may contain both)
     *                             3. A string containing the content of a PEM encoded certificate/key
    ...
    ) : string
    {
        $logger       = LoggerDepot::getLogger( get_called_class());
        $logger->log( LogLevel::DEBUG, self::$INIT . self::getCm( __METHOD__ ));
        list( $infileName, $outfileName ) =
            self::getInOutFiles( $stringToDecrypt, self::getUniqueFilenameBase());
        $outputString = null;
        try {
     ...
     *                                              The named file must contain a (single) PEM encoded certificate
     *                                           3. A string containing the content of a (single) PEM encoded certificate
     * @param null|array     $headers            Headers that will be prepended to the data after it has been encrypted
     *                                           1. an associative array keyed by header name
     *                                           2. an indexed array, where each element contains a single header line.
     * @param null|int       $flags              Opt. options that affect the encoding process (https://www.php.net/manual/en/openssl.pkcs7.flags.php)
     * @param null|int       $cipherId           One of cipher constants. (https://www.php.net/manual/en/openssl.ciphers.php)
     * @return bool
     * @throws InvalidArgumentException
     * @throws RuntimeException
    ...
    ) : bool
    {
        ...
        if( is_array( $recipCerts )) {
            foreach( $recipCerts as $x => $recipCert ) {
                $recipCerts[$x] = OpenSSLX509Factory::assertX509(
                    $recipCert,
                    3,
                    true,
                    false
            ...
            $recipCerts = OpenSSLX509Factory::assertX509(
                $recipCerts,
                3,
                true,
                false
    ...
    ) : string
    {
        ...
        $logger->log( LogLevel::DEBUG, self::$INIT . self::getCm( __METHOD__ ));
        list( $infileName, $outfileName ) =
            self::getInOutFiles( $stringToEncrypt, self::getUniqueFilenameBase());
        $outputString = null;
        try {
            self::encrypt(
                $infileName,
     ...
     *                                 where $key represents a key specified using the file or textual content notation above,
     *                                 and $passphrase represents a string containing the passphrase for that private key
     * @param null|array $headers    an array of headers that will be prepended to the data after it has been signed
     *                               1. an associative array keyed by header name
     *                               2. an indexed array, where each element contains a single header line
     * @param null|int   $flags      Opt. options that affect the encoding process (https://www.php.net/manual/en/openssl.pkcs7.flags.php)
     *                               default PKCS7_DETACHED
     * @param null|string $extraCerts
    ...
    ) : string
    {
        ...
        $logger->log( LogLevel::DEBUG, self::$INIT . self::getCm( __METHOD__ ));
        list( $infileName, $outfileName ) =
            self::getInOutFiles( $stringToSign, self::getUniqueFilenameBase());
        $outputString = null;
        try {
            self::sign(
                $infileName,
     ...
     * @param null|string $content filename, Will be filled with the verified data, but with the signature information stripped
     * @return bool
     * @throws InvalidArgumentException
     * @throws RunTimeException
     * @todo test
     ...
     *                               then it must be a correctly formed hashed directory
     *                               as the openssl command would use.
     * @param null|string $extraCerts    The filename of a file containing a bunch of certificates to use as untrusted CAs
     * @param bool   $result
    ...
    ) : array
    {
        ...
        list( $infileName, $contentFileName ) =
            self::getInOutFiles( $stringToVerify, $unique );
        $contentString    = null;
     ...
     * @throws RuntimeException
     */
    private static function getInOutFiles( string $infileContent, string $unique ) : array
    {
        static $IN   = '-IN';
        static $OUT  = '-OUT';
        static $TXT  = 'txt';
